...
# Agent prompt
SYSTEM_PROMPT = """You are a helpful Databricks assistant that can explore and query the Unity Catalog structure.
...
def format_messages_for_langchain(messages: List[ChatCompletionMessageParam]) -> List[Dict[str, Any]]:
    ...
    return {
      'response': {
        'choices': [{
          'message': {
            'role': 'assistant',
            'content': 'Please provide a question or request.'
          }
        }]
      }
    }
